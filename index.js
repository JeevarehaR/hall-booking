...
app.post("/createaRoom", function (request, response) {
  const { roomID, roomName, seatsAvailable, amenities, bookingDetails } =
    request.body;
  rooms.push({
    roomID,
    roomName,
...
app.post("/bookaRoom", function (request, response) {
  let { customerName, reservationDate, startTime, endTime, roomID } =
    request.body;
    ...
    if (rooms[i].roomID === roomID) {
      ...
      isBooked = rooms[i].bookingDetails.every((booking) => {
        ...
        return (
          ...
          ((startTS > endBookedTS && endTS > endBookedTS) ||
